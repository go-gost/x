...
func (h *relayHandler) handleConnect(ctx context.Context, conn net.Conn, network, address string, ro *xrecorder.HandlerRecorderObject, log logger.Logger) (err error) {
	...
	if h.md.sniffing {
		...
		handled, err := sniffing.Dispatch(sniffer, ctx, "tcp", conn, proto,
		...
		if handled {
			// Reset the recording start time: the sniffer handles the
			// connection and manages ro.Time from here on.
			ro.Time = time.Time{}
			return err
		}
	}

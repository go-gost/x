...
func TestSelectTarget_FromContextHost(t *testing.T) {
	h := newInitdHandler()
	ctx := ictx.ContextWithMetadata(context.Background(), xmd.NewMetadata(map[string]any{
		"host": "10.0.0.99:8080",
	}))

	target := h.selectTarget(ctx, "")
	if target == nil {
		t.Fatal("expected non-nil target")
	}
	if target.Addr != "10.0.0.99:8080" {
		t.Errorf("expected addr '10.0.0.99:8080', got '%s'", target.Addr)
	}
}

func TestSelectTarget_FromHop(t *testing.T) {
	h := newInitdHandler()
	h.hop = &mockHop{
		selectFn: func(ctx context.Context, opts ...hop.SelectOption) *chain.Node {
			return &chain.Node{Addr: "10.0.0.2:80", Name: "hop-node"}
		},
	}

	target := h.selectTarget(context.Background(), "")
	if target == nil {
		t.Fatal("expected non-nil target")
	}
	if target.Addr != "10.0.0.2:80" {
		t.Errorf("expected addr '10.0.0.2:80', got '%s'", target.Addr)
	}
}

func TestSelectTarget_NilHop(t *testing.T) {
	h := newInitdHandler()

	target := h.selectTarget(context.Background(), "")
	if target != nil {
		t.Error("expected nil target when no host and no hop")
	}
}

func TestSelectTarget_ContextHostOverridesHop(t *testing.T) {
	h := newInitdHandler()
	h.hop = &mockHop{
		selectFn: func(ctx context.Context, opts ...hop.SelectOption) *chain.Node {
	...
	if target.Addr != "10.0.0.99:8080" {
		t.Errorf("expected host from context to win, got '%s'", target.Addr)
	}
}
...
func TestHandleRawForwarding_NilTarget(t *testing.T) {
	h := newInitdHandler()
	conn := newStringConn(nil)
	ro := &xrecorder.HandlerRecorderObject{}
